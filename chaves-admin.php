<?php
	...
	if(isset($_POST['numero']) || isset($_POST['nome']) || isset($_POST['tipo'])){
		if(strlen($_POST['numero']) == 0){
			$resultado = '<div class="alert alert-danger">O <b>número</b> não foi preenchido!</div>';
			...
			$verificarChave = $mysqli->query("SELECT * FROM chaves WHERE numero = '$numero'") or die("Falha na execução do código SQL: ".$mysqli->error);
			$quantidade = $verificarChave->num_rows;

			if($quantidade != 0){
				$resultado = '<div class="alert alert-danger">A chave preenchida já foi cadastrada!</div>';
			}else{
				if(mysqli_query($mysqli, "INSERT INTO chaves(numero, nome, tipo) VALUES('$numero', '$nome', '$tipo')")){
					$resultado = '<div class="alert alert-success">Chave cadastrada com sucesso!</div>';
				}else{
					$resultado = '<div class="alert alert-danger">Houve um erro ao cadastrar a chave!</div>';
				}
			}
		}
...
<html>
	<head>
		...
		<title>Arachne - Cadastrar Chaves</title>
	</head>
				...
				<div class="container-fluid">
					...
					<div class="collapse navbar-collapse" id="navbarSupportedContent">
						<ul class="navbar-nav me-auto mb-2 mb-lg-0">
							...
							<li class="nav-item">
								<a class="nav-link active" aria-current="page" href="chaves-admin.php">Chaves</a>
							</li>
							<li class="nav-item">
				...
				<div class="row p-0">
					<div class="col m-0">
						<div class="bg-dark p-2 text-white">
							<span class="h6">Cadastro de Chaves</span>
						</div>
						<form class="border mb-3 pt-0 p-3" style="background-color: #f5f5f5;" action="" method="POST">
							<div class="row">
								<div class="col">
									<label class="mt-2" for="numero">Número:</label>
									<input id="numero" name="numero" type="text" class="form-control mb-0" placeholder="Ex: 1.0, 6.0..." required>
								</div>
								<div class="col">
									<label class="mt-2" for="nome">Nome:</label>
									<input id="nome" name="nome" type="text" class="form-control mb-0" placeholder="Ex: SA, SSTI..." required>
								</div>
							</div>
							<div class="row">
								<div class="col">
									...
									<select id="tipo" name="tipo" autocomplete="off" required>
										...
										<option value="Lacrada">Lacrada</option>
									</select>
								</div>
							</div>
							<div class="row mt-3">
							...
							<table class="table table-hover table-bordered text-center align-middle my-0">
								<thead>
									<tr>
										...
										<th style="background-color: #f5f5f5;" scope="col">Tipo</th>
										...
										if($result = $mysqli->query("SELECT * FROM chaves ORDER BY numero;")){
											while($row = $result->fetch_assoc()){													
												...
												<tr>
													...
													<td style="background-color: #fff;">'.$row["tipo"].'</td>
