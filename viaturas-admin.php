<?php
	...
	if(isset($_POST['registro']) || isset($_POST['modelo'])){
		if(strlen($_POST['registro']) == 0){
			$resultado = '<div class="alert alert-danger">O <b>registro</b> não foi preenchido!</div>';
			...
			$verificarViatura = $mysqli->query("SELECT * FROM viaturas WHERE registro = '$registro'") or die("Falha na execução do código SQL: ".$mysqli->error);
			$quantidade = $verificarViatura->num_rows;

			if($quantidade != 0){
				$resultado = '<div class="alert alert-danger">A viatura preenchida já foi cadastrada!</div>';
			}else{
				if(mysqli_query($mysqli, "INSERT INTO viaturas(registro, modelo) VALUES('$registro', '$modelo')")){
					$resultado = '<div class="alert alert-success">Viatura cadastrada com sucesso!</div>';
				}else{
					$resultado = '<div class="alert alert-danger">Houve um erro ao cadastrar a viatura!</div>';
				}
			}
		}
...
<html>
	<head>
		...
		<title>Arachne - Cadastrar Viaturas</title>
	</head>
				...
				<div class="container-fluid">
					...
					<div class="collapse navbar-collapse" id="navbarSupportedContent">
						<ul class="navbar-nav me-auto mb-2 mb-lg-0">
							...
							<li class="nav-item">
								<a class="nav-link active" aria-current="page" href="viaturas-admin.php">Viaturas</a>
							</li>
							<li class="nav-item">
				...
				<div class="row p-0">
					<div class="col m-0">
						<div class="bg-dark p-2 text-white">
							<span class="h6">Cadastro de Viaturas</span>
						</div>
						<form class="border mb-3 pt-0 p-3" style="background-color: #f5f5f5;" action="" method="POST">
							<div class="row">
								<div class="col">
									...
									<input id="registro" name="registro" type="text" class="form-control mb-0" placeholder="Ex: 14BC060, 12DP214..." required>
								</div>
								<div class="col">
									<label class="mt-2" for="modelo">Modelo:</label>
									<input id="modelo" name="modelo" type="text" class="form-control mb-0" placeholder="Ex: Fiorino, Frontier..." required>
								</div>
							</div>
							<div class="row mt-3">
								<div class="col">
